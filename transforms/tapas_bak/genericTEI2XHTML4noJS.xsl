...
<xsl:stylesheet version="1.0"
  ...
  <xsl:param name="tapasGenericCSS" select="concat($filePrefix,'/sites/all/modules/tapas_transform/transforms/tapas/css/tapasG.css')"/>
  <xsl:param name="customCSS" select="concat($filePrefix,'/sites/all/modules/tapas_transform/transforms/tapas/css/custom.css')"/>
  ...
  <xsl:variable name="htmlFooter">
    <div id="footer">
      <p>This is a pre-alpha version of the <a href="{$tapasHome}">TAPAS</a> generic
        view, based on TEI Boilerplate by John Walsh.</p>
    </div>
  </xsl:variable>
  ...
  <xsl:template name="contextual">
    <!-- Note: this routine may generate both an attribute and content, so it must be -->
    <!-- called before any other content of the currently output element has been generated -->
